...
class VishwamaiShaalaTrainer:
    ...
    def train_step(
        ...
        # Set up Aim tracking if enabled
        if self.cfg.monitoring.aim.enabled:
            aim_run = aim.Run(experiment=self.cfg.monitoring.aim.experiment_name)
        ...
        def loss_fn(params):
            ...
            if self.cfg.monitoring.aim.enabled and step % self.cfg.monitoring.log_every_n_steps == 0:
                for loss_name, loss_value in losses.items():
                    aim_run.track(float(loss_value), name=loss_name, step=step)
                
                # Log model artifacts if configured
                if self.cfg.monitoring.aim.log_artifacts and step % self.cfg.monitoring.save_every_n_steps == 0:
                    aim_run.track_text(str(self.cfg), name="config", step=step)
                    # Save model checkpoint
                    checkpoint_path = f"checkpoint_{step}"
                    state.save(checkpoint_path)
                    aim_run.track_artifacts(checkpoint_path, name="checkpoints")
